        ...
        <div className="text-center">
          <div className="flex flex-col gap-4">
            ...
            <h2 className="text-white sm:text-5xl text-3xl font-medium">
              Start investing with Wertchain today
            </h2>
          </div>
          <div className="mt-16 border border-border grid lg:grid-cols-3 sm:grid-cols-2 py-16 gap-10 px-20 rounded-3xl sm:bg-perk bg-dark_grey/35 lg:bg-bottom bg-center bg-no-repeat">
            {perksData.map((item, index) => (
              <Card key={index} className="text-center flex items-center justify-end flex-col bg-transparent border-none shadow-none ring-0 p-0">
                <CardContent className="p-0 flex flex-col items-center justify-end">
                  ...
                  <h3 className={`text-white text-28 mb-4 ${item.space}`}>{item.title}</h3>
                  <div className="text-muted/60" dangerouslySetInnerHTML={{ __html: item.text }} />
                </CardContent>
              </Card>
            ))}
          </div>
